...
import inventoryManagementData from "../temp/inventory_management_data.json";
...
const InventoryManagement = () => {
  ...
  const totalPages = Math.ceil(inventoryManagementData?.length / itemPerPage);

  const startIndex = (currentPage - 1) * itemPerPage;
  const endIndex = startIndex + itemPerPage;
  const currentItems = inventoryManagementData?.slice(startIndex, endIndex);
  ...
  return (
    <div className="flex flex-col gap-5 px-4 relative">
      <div className="flex justify-between items-center flex-wrap space-y-4 2xl:pr-8">
        <div className="font-medium text-2xl">Inventory Management</div>
        <div className="flex items-center justify-start md:justify-center gap-5 flex-wrap">
          <div className="search relative">
            <input
              ...
              placeholder="Device ID, Device Model, or Device Type"
            />
                ...
                <td className="border border-gray-300 px-4 py-2">
                  <div
                    ...
                    {actionDropDown === index && (
                      <div className="absolute bg-white py-3 w-52 -top-10 left-20 flex flex-col gap-2 font-medium shadow-lg">
                        <div className="hover:scale-[1.01] hover:bg-gray-100 px-5 py-1 w-full rounded cursor-pointer">
                          View detailed device
                        </div>
                        <div className="hover:scale-[1.01] hover:bg-gray-100 px-5 py-1 rounded cursor-pointer text-red-500">
                          Suspend Device
                        </div>
                      </div>
                    )}
                 ...
                 <div className="center">
                  <div
                    className={`w-28 p-1 center rounded-3xl ${
                      ...
                      item?.deviceStatus === "disconnected" &&
                      "bg-red-100"
                    } ${
                    ...
                    {" "}
                    {item?.deviceStatus?.slice(0, 1)?.toUpperCase() +
                    item?.deviceStatus?.slice(1)}
                  </div>
                  </div>
                </td>
              </tr>
      ...
      <div className="flex justify-between px-12 items-center">
        ...
        <div className="text-sm">
          ...
          {Math.min(endIndex, inventoryManagementData?.length)} of{" "}
          {inventoryManagementData?.length} entries
        </div>
      </div>
    </div>
  );
